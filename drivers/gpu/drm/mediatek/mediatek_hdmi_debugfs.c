...
static int mtk_hdmi_process_cmd(char *cmd, struct mediatek_hdmi *hdmi)
{
	char *np;
	unsigned long res;
	...
	if (0 == strncmp(cmd, "getedid", 7)) {
		...
		mtk_hdmi_info("cur dispaly: name:%s, hdisplay:%d\n",
			      hdmi->display_node->mode.name,
			      hdmi->display_node->mode.hdisplay);
		mtk_hdmi_info("hsync_start:%d,hsync_end:%d, htotal:%d",
			      hdmi->display_node->mode.hsync_start,
			      hdmi->display_node->mode.hsync_end,
			      hdmi->display_node->mode.htotal);
		mtk_hdmi_info("hskew:%d, vdisplay:%d\n",
			      hdmi->display_node->mode.hskew,
			      hdmi->display_node->mode.vdisplay);
		mtk_hdmi_info("vsync_start:%d, vsync_end:%d, vtotal:%d",
			      hdmi->display_node->mode.vsync_start,
			      hdmi->display_node->mode.vsync_end,
			      hdmi->display_node->mode.vtotal);
		mtk_hdmi_info("vscan:%d, flag:%d\n",
			      hdmi->display_node->mode.vscan,
			      hdmi->display_node->mode.flags);
		mtk_hdmi_info("current display mode:%s\n",
			      hdmi->dvi_mode ? "dvi" : "hdmi");
		mtk_hdmi_info("min clock:%d, max clock:%d\n",
			      hdmi->min_clock, hdmi->max_clock);
		mtk_hdmi_info("max hdisplay:%d, max vdisplay:%d\n",
			      hdmi->max_hdisplay, hdmi->max_vdisplay);
		mtk_hdmi_info("ibias:0x%x, ibias_up:0x%x\n",
		...
		if (res >= ARRAY_SIZE(display_mode)) {
			mtk_hdmi_err("invalid format, res = %ld\n",
				     res);
			goto errcode;
		}
...
int mtk_drm_hdmi_debugfs_init(struct mediatek_hdmi *hdmi)
{
	hdmi->debugfs =
...
void mtk_drm_hdmi_debugfs_exit(struct mediatek_hdmi *hdmi)
{
