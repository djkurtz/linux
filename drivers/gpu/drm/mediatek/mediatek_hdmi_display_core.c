			...
			struct device_node *np, void *data)
{
	...
	display_node =
	    kzalloc(sizeof(*display_node), GFP_KERNEL);
	if (!display_node) {
...
int mtk_hdmi_display_init(struct mediatek_hdmi_display_node *display_node)
{
		...
		if (node != NULL && node->display_ops != NULL &&
			...
			ret &=
			    node->display_ops->set_format(mode,
							  node->private_data);
			memcpy(&node->mode, mode, sizeof(*mode));
		}
		node = mtk_hdmi_display_get_next_node(node);
	}
